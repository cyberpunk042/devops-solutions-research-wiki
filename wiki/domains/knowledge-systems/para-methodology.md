...
- What is the right trigger for moving a wiki domain from "Areas" to "Archives" status — inactivity threshold, explicit decision, or relevance scoring?
- Should `decisions/` be a sub-layer of Projects (each decision was a project) or Areas (ongoing decision framework)?
- How should the CODE workflow's Express phase be operationalized for this wiki — beyond export profiles, what constitutes "expression" for a knowledge system?
- Is progressive summarization best implemented as page sections, or should separate summary artifacts exist per domain (FAQ pages as Forte's "executive summary" layer)?
