...
- **What is the empirical routing split after Stage 3?** The 80% target is an estimate. Actual measurement requires Stage 3 to be operational.
- **Can the wiki post-chain run entirely on local models?** Stage 3 targets this, but validation of complex relationship synthesis may still require Claude.
- **Context window as a hidden routing constraint.** Qwen3-8B has a 32K context limit; tasks involving full wiki analysis exceed this. How should the router handle context overflow — split the task, or escalate to cloud?
- **Profile auto-selection.** Should the pipeline auto-select the AICP profile based on the operation being run, or should profiles be set manually per session?
